...
export class PHPBackendService extends BackendService{
    ...
    override init(backend: string): void {
        ...
        this.listDataEndpoint = '/leer.php';
        this.newDataEndpoint = '/grabar.php';
        this.deleteDataEndpoint = '/borrar.php';
    }
    ...
    public getListUrl():string{
      ...
      public getDeleteDataUrl(id:number):string{
        if(this.backendUrl && this.deleteDataEndpoint)
          return `${this.backendUrl + this.deleteDataEndpoint}?id=${id}`;
        throw new Error('No se ha iniciado el servicio');
      }
}